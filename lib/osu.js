...
// Fetch a beatmapset (used as a fallback when a set ID is entered instead of a diff ID).
export async function getBeatmapset(id) {
  const token = await getAppToken();
  const res = await fetch(`https://osu.ppy.sh/api/v2/beatmapsets/${encodeURIComponent(id)}`, {
    headers: { Authorization: "Bearer " + token },
  });
  if (!res.ok) return null;
  return res.json();

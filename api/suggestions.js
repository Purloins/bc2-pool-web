...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === "POST") {
      if (!canSuggest) return res.status(403).json({ error: "Mappoolers only" });
      const b = req.body || {};
      if (!b.link && !b.title) return res.status(400).json({ error: "link or title required" });
      const item = {
        ...
        title: b.title || "",
        comment: b.comment || "",
        // auto-filled stats (optional)
        artist: b.artist || "",
        mapper: b.mapper || "",
        difficulty: b.difficulty || "",
        beatmapId: b.beatmapId || null,
        beatmapsetId: b.beatmapsetId || null,
        stars: b.stars ?? null,
        bpm: b.bpm ?? null,
        length: b.length || "",
        cs: b.cs ?? null,
